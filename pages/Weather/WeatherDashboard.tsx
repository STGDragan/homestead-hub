...
export const WeatherDashboard: React.FC = () => {
  ...
  const [locationInfo, setLocationInfo] = useState<{ zip: string, zone: string }>({ zip: 'Local', zone: 'Unknown' });
  ...
  const loadData = async () => {
    const f = await weatherService.getForecast();
    const a = await weatherService.getAlerts();
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
           ...
           <p className="text-earth-600 flex items-center gap-2 mt-1">
              <MapPin size={14} /> 
      ...
      {alerts.length > 0 && (
         <div className="space-y-2">
            {alerts.map(alert => (
               <div key={alert.id} className={`p-4 rounded-xl border flex gap-3 ${alert.level === 'warning' ? 'bg-red-50 border-red-200 text-red-900' : 'bg-amber-50 border-amber-200 text-amber-900'}`}>
      ...
      {/* Forecast Strip */}
      <section>
         <h2 className="text-lg font-bold text-earth-800 mb-4">7-Day Forecast</h2>
      ...
      <section>
         <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold text-earth-800">Microclimate Sensors</h2>
