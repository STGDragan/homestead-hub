...
import { Subscription, SubscriptionPlan, TrialCode, SubscriptionLog, UserProfile, FeatureAccessCache, Invoice } from '../types';
...
export const billingService = {
    ...
    async createInvoice(sponsorId: string, campaignId: string, amountCents: number, notes?: string): Promise<string> {
    ...
    async getInvoices(sponsorId?: string): Promise<Invoice[]> {
        if (sponsorId) {
            const all = await dbService.getAllByIndex<Invoice>('invoices', 'sponsorId', sponsorId);
