...
interface LogProductionModalProps {
  herdId: string;
  onSave: (log: ProductionLog) => void;
  onClose: () => void;
}

export const LogProductionModal: React.FC<LogProductionModalProps> = ({ herdId, onSave, onClose }) => {
      ...
      <div className="bg-white dark:bg-stone-900 rounded-2xl shadow-xl max-w-md w-full p-6 animate-in zoom-in-95 border border-earth-200 dark:border-stone-800">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-serif font-bold text-earth-900 dark:text-earth-100 flex items-center gap-2">
             <Egg className="text-amber-500" /> Log Production
          </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
           ...
           <TextArea label="Notes" value={notes} onChange={e => setNotes(e.target.value)} placeholder="Quality, issues, etc." />
