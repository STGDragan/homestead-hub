...
import { Sprout, Check, MapPin, User, Star, Loader2, AlertCircle, Mail, Lock, Phone } from 'lucide-react';
...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ onComplete }) => {
  ...
  const [formData, setFormData] = useState<Partial<UserProfile> & { email?: string; password?: string, phone?: string }>({
    name: '',
    zipCode: '',
    hardinessZone: '',
  ...
  const handleFinish = async () => {
     try {
         // Pass explicit user data (name, phone) to register so it hits Auth Metadata
         ...
         // Update the local profile store with all details
  ...
  const renderBasics = () => (
     <div className="space-y-4 max-w-md mx-auto">
        ...
        {/* Phone (Optional but good for auth) */}
        <Input 
